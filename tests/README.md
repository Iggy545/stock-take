...
| `web-sold-alert.js` | The flash that sends somebody to the shelves when a web sale arrives from another till: what raises one and what must never, the recency window that stops a reconnect replaying the shop's whole history, and being told the same order twice | v1.30.0 |
